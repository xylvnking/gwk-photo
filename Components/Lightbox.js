...
export default function Lightbox(props) {
    ...
    const handleInput = (input) => {
        ...
        if (count.current > props.photos.length) {
            count.current = 0
        } else if (count.current < 0 ) {
            count.current = props.photos.length
        } else {
  ...
  return (
    ...
    <div id={styles["lightboxContainer2"]}>
        <h1 onClick={() => props.setLightboxOpen(false)}>EXIT</h1>
        <div className={styles.arrowButton} onClick={() => navigate('back')}>
        ...
        <div className={styles.arrowButton} onClick={() => navigate('forward')}>
